...
export default function PatientSettings() {
  ...
  const [password, setPassword] = useState({ newPw: '', confirm: '' });
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card p-6 mb-6 flex items-center gap-5">
        ...
        <div>
          ...
          <span className="text-xs bg-warm-50 text-warm-700 border border-warm-200 px-2.5 py-0.5 rounded-full font-semibold capitalize mt-1 inline-block">Patient</span>
        </div>
      </div>
      ...
      <div className="card p-6 mb-6">
        ...
        {profileMsg && <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm px-4 py-3 rounded-xl mb-4"><RiCheckLine />{profileMsg}</div>}
        {profileErr && <div className="bg-blush-50 border border-blush-200 text-blush-700 text-sm px-4 py-3 rounded-xl mb-4">{profileErr}</div>}
        <form onSubmit={handleProfileSave} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">Full Name</label>
            <input value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} placeholder="Your full name" className="input-field" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">Email Address</label>
            <input type="email" value={profile.email} onChange={e => setProfile(p => ({ ...p, email: e.target.value }))} placeholder="your@email.com" className="input-field" required />
          </div>
      ...
      <div className="card p-6">
        ...
        {pwMsg && <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm px-4 py-3 rounded-xl mb-4"><RiCheckLine />{pwMsg}</div>}
        {pwErr && <div className="bg-blush-50 border border-blush-200 text-blush-700 text-sm px-4 py-3 rounded-xl mb-4">{pwErr}</div>}
        <form onSubmit={handlePasswordSave} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">New Password</label>
            <input type="password" value={password.newPw} onChange={e => setPassword(p => ({ ...p, newPw: e.target.value }))} placeholder="At least 6 characters" className="input-field" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">Confirm New Password</label>
            <input type="password" value={password.confirm} onChange={e => setPassword(p => ({ ...p, confirm: e.target.value }))} placeholder="Repeat new password" className="input-field" required />
          </div>
          <div className="flex justify-end">
