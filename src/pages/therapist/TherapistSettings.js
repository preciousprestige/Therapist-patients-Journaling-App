...
export default function TherapistSettings() {
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      const res = await api.put(`/users/${user._id}`, { name: profile.name, email: profile.email, bio: profile.bio });
      const updated = { ...user, name: res.data.name, email: res.data.email, bio: res.data.bio };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-stone-500 text-sm mt-1">Manage your account and public profile</p>
      </div>
      ...
      <div className="card p-6 mb-6 flex items-center gap-5">
        ...
        <div>
          ...
          <span className="text-xs bg-sage-50 text-sage-700 border border-sage-200 px-2.5 py-0.5 rounded-full font-semibold capitalize mt-1 inline-block">Therapist</span>
        </div>
      </div>
      ...
      <div className="card p-6 mb-6">
        ...
        <form onSubmit={handleProfileSave} className="space-y-4">
          <div>
            ...
            <input value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} className="input-field" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">Email Address</label>
            <input type="email" value={profile.email} onChange={e => setProfile(p => ({ ...p, email: e.target.value }))} className="input-field" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-1.5">
              Bio / About Me
              <span className="text-stone-400 font-normal ml-1 text-xs">(visible to patients when choosing a therapist)</span>
            </label>
            <textarea
              value={profile.bio}
              onChange={e => setProfile(p => ({ ...p, bio: e.target.value }))}
              placeholder="Tell patients about your background, approach, and what you specialize in..."
